...
(function (root, factory) {
  ...
  // The resolve/report contract (README, api/_hubspot.js) is camelCase fixPlan.
  // Older or varied payloads used snake_case / aliases; keep those as fallbacks.
  // Nullish checks so an explicit empty array does not fall through to aliases.
  function readFixPlan(data) {
    if (!data || typeof data !== 'object') return [];
    if (data.fixPlan != null) return data.fixPlan;
    if (data.fix_plan != null) return data.fix_plan;
    if (data.fixes != null) return data.fixes;
    if (data.recommendations != null) return data.recommendations;
    return [];
  }
